...
```
O0 OR A0 B0       # 至少一个事件发生
D0 AND A0 B0      # 两个事件同时发生
TOP AND O0 SUM    # 门定义顺序不影响结论，子门可在引用之后定义
```

顶事件：单个门名称。页面默认载入八组双元事件（`A0/B0` … `A7/B7`）的完整示例：
八个单组 OR 门、八个双事件 AND 门、一个汇总门，顶门同时引用八个 OR 门与汇总门，
其结论为 `8 × 2^7 = 1024` 个互不相同的九事件极小割集，16 个事件全部为可选。
...
- 结论只取决于门的逻辑结构，与门定义行的先后顺序无关（内部按分支数排序展开，仅影响性能）。
...
3. `scripts/verify-scenarios.ts` 规定场景断言：
   ...
   - 八组双元事件：`8×2^7=1024` 个互异的九事件割集、16 个事件全部可选、各门计数正确，
     并在门定义重排（顶门置首、子门晚于引用）后结论完全一致；
...
src/core/
  ...
  *.test.ts     # 核心算法测试（28 项，含 1024 割集与门重排）
